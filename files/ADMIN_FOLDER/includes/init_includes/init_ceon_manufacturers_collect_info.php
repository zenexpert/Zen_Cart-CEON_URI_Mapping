...
if (defined('FILENAME_MANUFACTURERS') &&
    ...
    isset($_GET['action']) && $_GET['action'] == 'deleteconfirm') {

	if (!function_exists('zen_admin_demo') || !zen_admin_demo()) {
		$manufacturers_id = (int)zen_db_prepare_input($_POST['mID']);
		...
		$ceon_uri_mapping_admin->deleteConfirmHandler($manufacturers_id);
...
if (defined('FILENAME_MANUFACTURERS') &&
    $_SERVER['SCRIPT_NAME'] == DIR_WS_ADMIN . (! str_contains(FILENAME_MANUFACTURERS, '.php') ? FILENAME_MANUFACTURERS . '.php' : FILENAME_MANUFACTURERS) &&
    (isset($_SESSION['ceon_uri_mapping_manufacturers_insert']) || isset($_SESSION['ceon_uri_mapping_manufacturers_save']))) {
	...
	$ceon_uri_mapping_admin->insertSaveHandler($manufacturers_id, $manufacturers_name);
...
if (defined('FILENAME_MANUFACTURERS') &&
    $_SERVER['SCRIPT_NAME'] == DIR_WS_ADMIN . (! str_contains(FILENAME_MANUFACTURERS, '.php') ? FILENAME_MANUFACTURERS . '.php' : FILENAME_MANUFACTURERS) &&
    isset($_GET['action']) && ($_GET['action'] == 'insert' || $_GET['action'] == 'save')) {
